...
import saga.progetto.tesi.map.cell.ThirdTowerDecoration;
import saga.progetto.tesi.map.cell.ThirdTowerFloor;
import saga.progetto.tesi.map.cell.ThirdTowerWall;
import saga.progetto.tesi.navigable.Gameloop;

public class ThirdBossMap extends GameMap
{
	public static final int HEIGHT = 35;
	public static final int WIDTH = 35;
	
	public ThirdBossMap(Player player, TheKnowledgeTowers game, String mapId, String previousMap, Gameloop gameloop)
	{
	...
	protected void mapInit(MapData data, String previousMap)
	{
		...
		for (String entry : data.getCells())
		{
			...
			{
				cell = new ThirdTowerDecoration(Float.parseFloat(info[0]) + getWidthOffset(), Float.parseFloat(info[1]) + getHeightOffset(), this);
				getWalls().add(cell);				
			}

			else if (Boolean.parseBoolean(info[2]))
				cell = new ThirdTowerFloor(Float.parseFloat(info[0]) + getWidthOffset(), Float.parseFloat(info[1]) + getHeightOffset(), this);
			...
			else
			{
				...
				if (offsetX + offsetY == 0)
					cell = new ThirdTowerWall(Float.parseFloat(info[0]) + getWidthOffset(), Float.parseFloat(info[1]) + getHeightOffset(), this, true);
				
				else
					cell = new ThirdTowerWall(Float.parseFloat(info[0]) + getWidthOffset(), Float.parseFloat(info[1]) + getHeightOffset(), this);
...
//		updateBackground(getThiefAbilityLayer());
	...
	public void update(int delta)
	{
		...
		if (getEnemies().size() == 0 && getLocations().size() > 0 && !getLocations().get(getGameloop().getPreviousMap()).isAccessible())
			getLocations().get("3towerend").setAccessible(true);
	}
}
