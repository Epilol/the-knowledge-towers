...
import saga.progetto.tesi.map.cell.TenthTowerDecoration;
import saga.progetto.tesi.map.cell.TenthTowerFloor;
import saga.progetto.tesi.map.cell.TenthTowerWall;
import saga.progetto.tesi.navigable.Gameloop;

public class TenthBossMap extends GameMap
{
	public static final int HEIGHT = 35;
	public static final int WIDTH = 35;
	
	public TenthBossMap(Player player, TheKnowledgeTowers game, String mapId, String previousMap, Gameloop gameloop)
	{
	...
	protected void mapInit(MapData data, String previousMap)
	{
		...
		for (String entry : data.getCells())
		{
			...
			{
				cell = new TenthTowerDecoration(Float.parseFloat(info[0]) + getWidthOffset(), Float.parseFloat(info[1]) + getHeightOffset(), this);
				getWalls().add(cell);				
			}

			else if (Boolean.parseBoolean(info[2]))
				cell = new TenthTowerFloor(Float.parseFloat(info[0]) + getWidthOffset(), Float.parseFloat(info[1]) + getHeightOffset(), this);
			...
			else
			{
				...
				if (offsetX + offsetY == 0)
					cell = new TenthTowerWall(Float.parseFloat(info[0]) + getWidthOffset(), Float.parseFloat(info[1]) + getHeightOffset(), this, true);
				
				else
					cell = new TenthTowerWall(Float.parseFloat(info[0]) + getWidthOffset(), Float.parseFloat(info[1]) + getHeightOffset(), this);
...
//		updateBackground(getThiefAbilityLayer());
	...
	public void update(int delta)
	{
		...
		if (getEnemies().size() == 0 && getLocations().size() > 0 && !getLocations().get(getGameloop().getPreviousMap()).isAccessible())
			getLocations().get("10towerend").setAccessible(true);
	}
}
