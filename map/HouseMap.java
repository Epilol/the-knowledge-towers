...
import saga.progetto.tesi.map.cell.HouseDecoration;
import saga.progetto.tesi.map.cell.HouseFloor;
import saga.progetto.tesi.map.cell.HouseWall;
import saga.progetto.tesi.navigable.Gameloop;

public class HouseMap extends GameMap
{
	private static final int WIDTH = 35;
	private static final int HEIGHT = 35;
	
	public HouseMap(Player player, TheKnowledgeTowers game, String mapId, String previousMap, Gameloop gameloop)
	{
	...
	protected void mapInit(MapData data, String previousMap)
	{
		...
		for (String entry : data.getCells())
		{
			...
			{
				cell = new HouseDecoration(Float.parseFloat(info[0]) + getWidthOffset(), Float.parseFloat(info[1]) + getHeightOffset(), this);
				getWalls().add(cell);
			}

			else if (Boolean.parseBoolean(info[2]))
				cell = new HouseFloor(Float.parseFloat(info[0]) + getWidthOffset(), Float.parseFloat(info[1]) + getHeightOffset(), this);
			...
			else
			{
				...
				if (offsetX + offsetY == 0)
					cell = new HouseWall(Float.parseFloat(info[0]) + getWidthOffset(), Float.parseFloat(info[1]) + getHeightOffset(), this, true);
				
				else
					cell = new HouseWall(Float.parseFloat(info[0]) + getWidthOffset(), Float.parseFloat(info[1]) + getHeightOffset(), this);
