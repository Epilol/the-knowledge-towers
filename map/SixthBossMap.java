...
import saga.progetto.tesi.map.cell.SixthTowerDecoration;
import saga.progetto.tesi.map.cell.SixthTowerFloor;
import saga.progetto.tesi.map.cell.FifthTowerWall;
import saga.progetto.tesi.navigable.Gameloop;

public class SixthBossMap extends GameMap
{
	public static final int HEIGHT = 35;
	public static final int WIDTH = 35;
	
	public SixthBossMap(Player player, TheKnowledgeTowers game, String mapId, String previousMap, Gameloop gameloop)
	{
	...
	protected void mapInit(MapData data, String previousMap)
	{
		...
		for (String entry : data.getCells())
		{
			...
			{
				cell = new SixthTowerDecoration(Float.parseFloat(info[0]) + getWidthOffset(), Float.parseFloat(info[1]) + getHeightOffset(), this);
				getWalls().add(cell);				
			}

			else if (Boolean.parseBoolean(info[2]))
				cell = new SixthTowerFloor(Float.parseFloat(info[0]) + getWidthOffset(), Float.parseFloat(info[1]) + getHeightOffset(), this);
...
//		updateBackground(getThiefAbilityLayer());
	...
	public void update(int delta)
	{
		...
		if (getEnemies().size() == 0 && getLocations().size() > 0 && !getLocations().get(getGameloop().getPreviousMap()).isAccessible())
			getLocations().get("6towerend").setAccessible(true);
	}
}
