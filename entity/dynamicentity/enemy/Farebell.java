...
public class Farebell extends Enemy
{
	private static final IDimension FAREBELL_SIZE = new Dimension(32, 42);
	private static final String FAREBELL_PATH = "images/characters/enemies/farebell.png";
	private static final int FAREBELL_LEVEL = 999; // 3
	private static final float EXP_MODIFIER = 40.0f;
	private static final float FAREBELL_SPEED = 2.0f;
	...
	private static Image farebellImage;

	public Farebell(float x, float y, Direction direction, Player player, GameMap map, boolean hasBeenKilled, String id)
	{
		super(x, y, player, FAREBELL_SIZE, map, hasBeenKilled, id, TOTAL_LIFE);
		setSprite(new Sprite(farebellImage, getFrameDuration(), FAREBELL_SIZE.width(), FAREBELL_SIZE.height()));
		getSprite().setLastDirection(direction);
		setTotalEndurance(TOTAL_ENDURANCE);
		setCurrentEndurance(TOTAL_ENDURANCE);
		setBar(new NPCBar(this, FAREBELL_SIZE));
	...
	{
		farebellImage = assets().getImage(FAREBELL_PATH);
		watcher.add(farebellImage);
	}

	@Override
	public int getLevel()
	{
		return FAREBELL_LEVEL;
	}
...
//				golds = new Random().nextInt((int) (MAX_GOLDS - MIN_GOLDS)) + 1;
	...
	{
		return FAREBELL_SPEED;
	}
	
	@Override
	public IDimension getSize()
	{
		return FAREBELL_SIZE;
	}
