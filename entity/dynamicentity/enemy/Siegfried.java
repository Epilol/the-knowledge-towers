...
public class Siegfried extends Enemy
{
	private static final IDimension SIEGFRIED_SIZE = new Dimension(32, 47);
	private static final String SIEGFRIED_PATH = "images/characters/enemies/siegfried.png";
	private static final int SIEGFRIED_LEVEL = 999; // 3
	private static final float EXP_MODIFIER = 100.0f;
	private static final float SIEGFRIED_SPEED = 2.0f;
	...
	private static final float MAGICAL_RESISTANCE = 0.35f;
	private static Image siegfriedImage;

	public Siegfried(float x, float y, Direction direction, Player player, GameMap map, boolean hasBeenKilled, String id)
	{
		super(x, y, player, SIEGFRIED_SIZE, map, hasBeenKilled, id, TOTAL_LIFE);
		setSprite(new Sprite(siegfriedImage, getFrameDuration(), SIEGFRIED_SIZE.width(), SIEGFRIED_SIZE.height()));
		getSprite().setLastDirection(direction);
		setTotalEndurance(TOTAL_ENDURANCE);
		setCurrentEndurance(TOTAL_ENDURANCE);
		setBar(new NPCBar(this, SIEGFRIED_SIZE));
	...
	{
		siegfriedImage = assets().getImage(SIEGFRIED_PATH);
		watcher.add(siegfriedImage);
	}

	@Override
	public int getLevel()
	{
		return SIEGFRIED_LEVEL;
	}
	...
	{
		return SIEGFRIED_SPEED;
	}
	
	@Override
	public IDimension getSize()
	{
		return SIEGFRIED_SIZE;
	}
